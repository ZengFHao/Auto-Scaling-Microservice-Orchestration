...
def update_state(state,action,ms_idx):
    ...
    state_new[MS_NUM][act_idx] += MS_RESOURCE[str(ms_idx)]["cpu"]
    state_new[MS_NUM+1][act_idx] -= MS_RESOURCE[str(ms_idx)]["cpu"]
    state_new[MS_NUM+2][act_idx] += MS_RESOURCE[str(ms_idx)]["mem"]
    state_new[MS_NUM+3][act_idx] -= MS_RESOURCE[str(ms_idx)]["mem"]
    ...
    for user in user_request:
        ...
        for app in req:
            ...
            for ms in app:
                ...
                # per ms exe time
                exe_time = cal_exetime(ms_idx,iamge_num)  * 10
                app_time += exe_time
                # t_up & t_down & tp
                t_up = 0
                t_p = 0
...
def initial_ms():
    ...
    for ms_idx in range(MS_NUM):
        MS_RESOURCE[f"{ms_idx}"] = {"cpu": MS(ms_idx).cpu, "mem": MS(ms_idx).memory}
    request = user_request
    my_map = {i: 0 for i in range(MS_NUM)}
    total_num = 0
    for user in request:
        req = user_request[user]
        for msn in req:
            for ms in msn:
                my_map[ms.id] = my_map[ms.id] + 1
    for key, value in my_map.items():
        total_num += value
    threshold = math.ceil(value / MS_NUM)
    for key, value in my_map.items():
        if(value >= threshold):
            MS_RESOURCE[str(key)]["cpu"] *= 1.25
            MS_RESOURCE[str(key)]["mem"] *= 1.25
        else:
            MS_RESOURCE[str(key)]["cpu"] *= 0.75
            MS_RESOURCE[str(key)]["mem"] *= 0.75
