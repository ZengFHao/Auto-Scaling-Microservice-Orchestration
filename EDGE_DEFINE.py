...
memory_rate = 0.01
eta = 0.8
#默认user 15， node 4， chain 10， ms 8
# cpu_rate = 0.3
# memory_rate = 0.7
...
ms_image = [2,2,2,2,2,2,2,2,2,2,2,2] 
...
def get_mu():
    MU = []
    for i in range(MS_NUM):
        # MU.append(random.randint(3,5) * MS_RESOURCE[str(i)]["cpu"] * (MS_RESOURCE[str(i)]["mem"] / 10) ) 
        MU.append(random.randint(3,5) * MS(i).cpu * (MS(i).memory / 10) ) 
    return MU
