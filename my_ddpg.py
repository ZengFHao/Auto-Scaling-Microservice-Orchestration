"""
Reward Memory Shaping Deep Deterministic Policy Gradient
-----------------------------------------
tensorflow >=2.0.0
tensorlayer >=2.0.0
------
...
class my_ddpg(object):
    ...
    def __init__(self, s_dim, a_dim):
        ...
        def get_actor(input_state_shape, name=''):
            ...
            gru_out, _ = tl.layers.GRURNN(units=HIDDEN_SIZE, return_last_output=True)(inputs)
            gru_out = tl.layers.Reshape(shape=[-1, HIDDEN_SIZE], name='A_reshape')(gru_out)
            x = tl.layers.Dense(n_units=64, act=tf.nn.relu, W_init=W_init, b_init=b_init, name='A1')(gru_out)
        ...
        def get_critic(input_state_shape, input_action_shape, name=''):
            s = tl.layers.Input(input_state_shape, name='C_s_input')  # shape: (None, None, s_dim)
            a = tl.layers.Input(input_action_shape, name='C_a_input')  # shape: (None, None, a_dim)
            
            # 合并层
            concat = tl.layers.Concat(concat_dim=-1)([s, a])
            
            # GRU层（显式解包输出）
            gru_out, _ = tl.layers.GRURNN(
                units=HIDDEN_SIZE,
                return_last_output=True,
                return_last_state=True,  # 关键参数
                name='GRU_layer'
            )(concat)
            
            # 维度调整
            gru_flat = tl.layers.Reshape(shape=[-1, HIDDEN_SIZE])(gru_out)
            
            # Dense层修正
            x = tl.layers.Dense(n_units=64, act=tf.nn.relu,
                            W_init=W_init, b_init=b_init, name='C_l1')(gru_flat)
            x = tl.layers.Dense(n_units=1, W_init=W_init,
                            b_init=b_init, name='C_out')(x)
